...
async function getHistorical(req) {
    ...
    try {
        ...
        asset = await assetData.aggregate([{
            '$match': {
        ...
        let fDate;
        let tDate;
        //    console.log(new Date(Number(req.query.fDate)), new Date(Number(req.query.tDate)))
        ...
        if (req.query.view == 'table') {
            ...
            fDate12 = new Date((Number(req.query.fDate))).toLocaleString("en-US", {
                timeZone: asset.length > 0 ? asset[0].timeZone : "Asia/Kolkata",
                dateStyle: "medium",
                timeStyle: "long"
            })
            fromTime = fDate12.split(" ")
            fromTime.splice(3, 2, "00:00:00", "AM");
            timefrom = fromTime.join(" ");

            // console.log(timefrom);
            // console.log(Date.parse(timefrom));

            tDate12 = new Date((Number(req.query.tDate))).toLocaleString("en-US", {
                timeZone: asset.length > 0 ? asset[0].timeZone : "Asia/Kolkata",
                dateStyle: "medium",
                timeStyle: "long"
            })
            toTime = tDate12.split(" ")
            toTime.splice(3, 2, "11:59:59", "PM");
            timeto = toTime.join(" ");
            // console.log(timeto);
            // console.log(Date.parse(timeto));

            fDate = Date.parse(timefrom);
            tDate = Date.parse(timeto);
        } else if (req.query.view == 'realtime') {
        ...
        let getAllHistorical = req.query.view == 'table' ? await historicalDataNew.aggregate([
            ...
            },
            {
                '$lookup': {
                    ...
                    'as': 'companyAssetDetails'
                }
            },
            { $unwind: '$companyAssetDetails' },
            {
                '$project': {
                    '_id': "$_id",
                    ...
                    'data.ValueReceivedDate': {
                        $map: {
                            ...
                            in: {
                                $dateToString: {
                                    format: "%H:%M:%S",
                                    timezone: "$companyAssetDetails.timeZone",
                                    date: {
                                        $toDate: {
                                            $multiply: [
                                                "$$ts"
            ...
            {
                '$project': {
                    '_id': "$_id", 'data.ActValue': "$Data.ActValue", 'date': "$Date", 'data.ValueReceivedDate': "$Data.ValueReceivedDate", 'data.Name': "$parameterDetails.Name", 'data.ColorCode': "$parameterDetails.ColorCode", 'prameterId': "$parameterDetails._id", 'data.RegisterAddress': "$Data.registerDetails.RegisterAddress",
            ...
            },
            {
                '$project':
                {
                    ...
                    'ColorCode': 1,
                    'Maxlimit': 1,
                    'MaxAlert': { $last: "$alertlData.upperThresholdValue" },
                    'MinAlert': { $last: "$alertlData.lowerThresholdValue" },
                }
            },
            // },
...
// getGraphHistoricalRange
async function getGraphHistoricalRange(req) {
    // console.log('get historical hitt',req.query);
    try {
        var historicalDataNew = new historicalData(req.query.companyId);
        asset = await assetData.aggregate([{
        ...
        const getAllHistorical = await historicalDataNew.aggregate([
            ...
            },
            {
                '$project': {
                    ...
                    'data.RegisterAddress': { $last: "$Data.registerDetails.RegisterAddress" },
                    'data.ColorCode': "$parameterDetails.ColorCode",
                    'parameterId': "$parameterDetails._id",
                    'group': { $filter: { input: "$group", as: "u", cond: { $eq: ["$$u.assetId", req.query.assetId] } } }
                }
            },
            { $sort: { date: 1 } },
        ...
        getAllHistorical.forEach((element, i) => {
            element.data.forEach((element1, i1) => {
                // console.log(element1, i1)
                let obj = { data: [], name: '', registerAddress: '' }
                const allActValue = _.flatMap(element1, 'ActValue');
                const allValueReceivedDate = _.flatMap(element1, 'ValueReceivedDate');
                const result = _.zip(allValueReceivedDate, allActValue);
                obj = { data: result, name: element1[0].Name[0], registerAddress: element1[0].RegisterAddress }
                getAllHistorical[i].data[i1] = obj
                obj = { data: [], name: '', registerAddress: '' }
            })
