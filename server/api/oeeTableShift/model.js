...
const oeeTableSchema = new Schema({
    ...
    ShiftId : {type : mongoose.Schema.Types.ObjectId, default : 80},
});
oeeTableSchema.index({ AssetId: 1 });
oeeTableSchema.index({ AssetId: 1 ,Date: 1}, { unique: true });
oeeTableSchema.index({ AssetId: 1 ,ShiftId: 1, Date: 1}, { unique: true });
var oeeTableData = mongoose.model("oeetableshift",oeeTableSchema);
