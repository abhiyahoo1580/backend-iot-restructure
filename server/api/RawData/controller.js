const rawDataData = require('./model');
const mongoose = require('mongoose');

async function insertRawData(req){
    // console.log("insertRawData hitt");
    try{
        const rawData = new rawDataData(req.body);
        let existRawData = await rawDataData.find({$and: [{rawDataName : req.body.rawDataName, isDeleted: false }]});
        if(existRawData.length){
            return {
                msg : "RawData already exist"
            }
        }else {
            const insertedRawData = await rawData.save();
            return {
