...
// getOEMidentify
async function getOEMidentify(req) {
    // console.log('get subject hitt',req.query);
    try {
        ...
        if (!token) {
            ...
            let value = jwt.verify(token, config.SECRET, function (err, decoded) {
                if (err) {
                    // console.log(err)
...
async function updateOEMUser(req) {
    ...
    try {
        ...
        //     }
        // }else { 
        const updatedDevice = await userData.findByIdAndUpdate(userId, {
            $set: {
